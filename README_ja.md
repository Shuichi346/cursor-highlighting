...
メニューバードロップダウンから設定ウィンドウを開きます。設定は4つのタブに整理されています：

### スポットライト

| 設定 | 説明 | デフォルト |
|---|---|---|
| スポットライトを有効にする | スポットライト効果を切り替え | オフ |
| アクティベーションホットキー | 切り替えのグローバルショートカット | `⇧1` |
| スポットライト半径 | 明るいエリアのサイズ | 150 px |
| エッジぼかし | 円のエッジの柔らかさ | 30 px |
| 背景不透明度 | 周囲画面の暗さ | 50% |
| スポットライト色 | スポットライト円の色合い | 白 |

### クリック効果

| 設定 | 説明 | デフォルト |
|---|---|---|
| クリックリングを有効にする | クリック視覚化を切り替え | オン |
| ホットキー | 切り替えのグローバルショートカット | なし |
...
| ホットキー | 切り替えのグローバルショートカット | なし |
| フォントサイズ | 表示されるキーテキストのサイズ | 48 pt |
| テーマ | ライトまたはダークHUD背景 | ダーク |
...
すべてのショートカットはグローバルで、フォアグラウンドのアプリケーションに関係なく動作します。内蔵のホットキーレコーダーを使用して、それぞれの設定タブでカスタマイズしてください。
