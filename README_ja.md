...
- **キーストローク表示** — 押されたキーを画面下部中央のHUDオーバーレイでネイティブmacOS修飾記号（`⌘` `⌥` `⇧` `⌃` `⇪` `fn`）と共に表示します。ライト/ダークテーマを選択可能で、キーボードショートカットのデモンストレーションに最適です。

- **グローバルホットキー** — すべての機能は、他のアプリがフォアグラウンドにある時でも動作する完全にカスタマイズ可能なグローバルキーボードショートカットで切り替え可能です。

- **マルチモニター対応** — すべてのオーバーレイが接続されたディスプレイ間でカーソルをシームレスに追従します。
...
メニューバーのドロップダウンから設定ウィンドウを開きます。設定はサイドバーナビゲーションで4つのタブに整理されています：

### スポットライト

| 設定 | 説明 | デフォルト |
|---|---|---|
| スポットライトを有効 | スポットライト効果を切り替え | オン |
| 有効化ホットキー | 切り替え用グローバルショートカット | *(なし)* |
| スポットライト半径 | 明るい領域のサイズ | 30 px |
| エッジブラー | 円のエッジの柔らかさ | 0 px |
| 背景の不透明度 | 周囲の画面の暗さ | 0% |
| スポットライト色 | スポットライト円のティント色 | 半透明の赤 |
...
├── Settings/                   # サイドバーナビゲーション付きカスタムSwiftUI設定ウィンドウ
